...
const CategoryCard = ({ Category }: Props) => {
  return (
    <div className='flex justify-between bg-slate-50 p-4 ml-2 mr-12 rounded-lg  hover:bg-slate-200'>
      <div className='flex '>
        ...
        <div>
          ...
          <p className='text-xs'>
            <span className='mr-2 '>সাবক্যাটাগরি:</span>
            {Category.no_of_subcat}
          </p>
        </div>
      </div>
      <div className='flex flex-col align-middle text-xs '>
        <span className='pl-2'>{Category.no_of_dua}</span> <span>দুয়া</span>
      </div>
    </div>
  );
};
